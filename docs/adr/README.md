...
> 2026-08-14：60 篇（+0062 资源来源分解两级分区），提取窗口延伸至 2026-08-14。
## 架构
...
| [0062](./0062-breakdown-sections.md) | 资源来源分解两级分区：固定产出 / 永久加成 sections + 折减独立区（拆跨周目永久行、补结盟/贡税、energyRatio 基线修正，守恒不改） |
...
- 0062 → 0016/0036/0027/0047/0053：来源分解两级分区是顶部资源条（0016 相对价格同域）的展示重构——按加法/乘数形态二分（0036 机制二分同构）；拆行不改变引擎数学（守恒）；区域加成混合行源自 0027/0047/0053 的永久加成通道（`permanentBonuses['production']` 单一字段，历史不可拆）。
