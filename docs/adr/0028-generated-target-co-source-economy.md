...
**攻占目标奖励/消耗小范围随机**（用户体验）：`generateConquestTarget` 生成时对奖励整体（矿+科技）与消耗整体（矿+能源）各乘独立随机因子 `1 + (roll×2−1)×GEN_CONQUEST_RANDOM_PCT`（±20%，`balance.ts`）——目标间数值有差异，消除「每个目标消耗/奖励一模一样」的单调。约束：① 确定性固化（fixed-rng 持久计数器，防 SL）；② 惰性重滚不随机（jitter 默认 1，幂等保持）；③ 净收益恒正（最差 reward×0.8 / cost×1.2×折扣 ≤ 0.5 → 比值 ≥ 2.67 > 0）；④ 科技点消耗不变（保持 ADR-0025「科技点=产出型资源」定位，消耗仍为矿+能源）。
