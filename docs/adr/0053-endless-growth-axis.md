...
2. **boss 军力挑战**：每 3 层（layer ≥ 3 且 layer % 3 === 0）出现一个 boss 攻占目标（`boss:L<layer>`），复用攻占结算管线（发起/守卫/结算/奖励）。守卫公式 `min(产能×40s×(1+0.15×(layer-1)), ⌊军力上限×1/3⌋×(1+0.10×(layer-1)), 产能×180s×2)`，受攻占双上限约束；奖励锚定当期净产出 × 层数系数。
