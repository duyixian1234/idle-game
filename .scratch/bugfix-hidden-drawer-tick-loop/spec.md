...
- **护航费（Bug 3）**：本 spec 出范围，但已按 Q3 锁定的方向单独落地（ADR-0044，提交同日）：`ESCORT_ENERGY_SECONDS` 10→1 + 单次护航费 ≤ 当前能源 50% 兜底（`startExpedition` 不足暂缓，reason 入 `AUTO_PAUSE_REASONS`），保留同杠杆防印钞不变量。存档模拟验证：能源 2h 最小 22.3兆 > 0（修复前周期性归零），矿物 2h 增 7.24e15。
