...
> **⚠️ 已被推翻（2026-08-08，[ADR-0037](../../docs/adr/0037-remove-bulk-buymax.md)）**：本 spec 建立的 buyMax 买满机制（`bulk.ts` + Shift 买满 + 确认弹窗 + 4 类批量）已全部移除。动机=简化心智负担（M1）+ 后期基础建造物购买需求消失。`bulk.ts` 整文件删除，6 个实现 ticket 的产物随之废弃。本 spec 保留作历史记录。
