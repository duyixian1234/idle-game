 ...
 * - 单槽：同时最多 1 支派遣，固定 60 分钟，离线照常推进，不可取消。
 * - 全提交：出发时扣资源（矿物/能源动态缩放 + 固定兵力锁定）+ 用 `explore` 域固定种子
 *   **roll 并固化结果**；回归只入账（`settleExpeditions`），防 SL 在结构上成立。
 * - 奖池剔除制：未发现势力（w2）+ 未发现天体（w1）+ 资源补偿（w = max(2, 6-已收集)），
...
/** 当前派遣消耗：矿物/能源随每秒产出动态缩放（带封顶），兵力固定 */
export function expeditionCost(state: GameState): { mineral: number; energy: number; military: number } {
  const prod = netProduction(state)
  return {
    mineral: scaledClamp(prod.mineral, EXPEDITION_MINERAL.min, EXPEDITION_MINERAL.factor, EXPEDITION_MINERAL.cap),
    energy: scaledClamp(prod.energy, EXPEDITION_ENERGY.min, EXPEDITION_ENERGY.factor, EXPEDITION_ENERGY.cap),
    military: EXPEDITION_MILITARY_COST,
  }
}
...
/**
 * 探索奖池（剔除制）：未发现的探索势力（各 w2）+ 未发现的探索天体（各 w1）
...
/** 资源补偿数值（按当前投入比例返还 + 科技点出口） */
function compensationFor(cost: { mineral: number; energy: number }): { mineral: number; tech: number; energy: number } {
  return {
    mineral: Math.floor(cost.mineral * EXPEDITION_COMPENSATE_RATIO.mineral),
    energy: Math.floor(cost.energy * EXPEDITION_COMPENSATE_RATIO.energy),
    tech: Math.floor(cost.mineral * EXPEDITION_COMPENSATE_RATIO.techPerMineral),
  }
}
 ...
 */
function rollFromPool(pool: ExpeditionPoolEntry[], roll: () => number, cost: { mineral: number; energy: number }): ExpeditionResult {
  ...
  for (const entry of pool) {
    ...
    if (value <= 0) {
      ...
      return { kind: 'resource', ...compensationFor(cost) }
    }
  }
  // 浮点边界兜底：最后一项
...
/**
 * 发起探索派遣（全提交语义）：
 * 校验（通关后 phase / 单槽 / 矿物/能源/兵力足够）→ 扣资源 → `explore` 域 roll 固化结果 → push。
 * rng 不传（undefined）→ 结果型随机走 explore 域持久化计数器（fixed-rng 防 SL）；
 * 显式传 rng → 测试注入（跳过计数器）。
 */
export function startExpedition(state: GameState, nowMs: number, rng?: () => number): ExpeditionActionResult {
  if (!isExploreAvailable(state)) return { ok: false, reason: '通关后开放探索' }
  if (state.expeditions.some((e) => !e.resolved)) return { ok: false, reason: '已有一支探索队在途中' }
  ...
  const result = rollFromPool(pool, rng ?? rollDomain(state, 'explore'), cost)
 ...
 * - faction：运行时创建派系（createFactionState，favor/threat 取 def 初值）+ 记录发现进度
 * - planet：解锁天体（{ unlocked: true, unlockedAt }）+ 记录发现进度
 * - resource：按出发时固化的补偿值入账（含科技点）
