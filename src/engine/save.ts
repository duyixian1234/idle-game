...
import { BUILDINGS, EXPLORE_FACTIONS, RESOURCE_KEYS } from './data'
...
const SAVE_SCHEMA: FieldSpec[] = [
  ...
  { key: 'explorationAlliances', since: SCHEMA_V18, check: isArray },
 ...
 *   否则旧档探索外交产出加成永远为 0（"改动不生效"的直接诱因）。
 * - 幂等：字段已存在（数组）则保留（跨 NG+ 的累计不被覆盖）。 */
function migrateV17ToV18(raw: Record<string, unknown>): Record<string, unknown> {
  const next = { ...raw }
  if (!isArray(next.explorationAlliances)) {
    const factions = isPlainObject(next.factions) ? (next.factions as Record<string, unknown>) : {}
    next.explorationAlliances = Object.keys(EXPLORE_FACTIONS).filter((id) => {
      const f = factions[id]
      return isPlainObject(f) && (f as { allied?: unknown }).allied === true
    })
  }
  next.schemaVersion = SCHEMA_V18
  return next
}
